<template>
    <v-container>
      <v-data-table :search="search" :headers="headers" :items="desserts" class="elevation-1">
        <template v-slot:top>
          <v-toolbar flat>
            <v-toolbar-title>Localidad</v-toolbar-title>
  ...
  export default {
    ...
    methods: {
      showData() {
        ...
        axios
          .get("http://localhost:8081/Localidad")
          .then((response) => {
            this.result = response.data.data;
            //console.log(response.data);
            for (let i = 0; i < response.data.length; i++) {
              this.desserts.push({
                codigo: response.data[i].clocalidad,
      ...
      saveData: function () {
        if (this.editedIndex > -1) {
          axios
            .put(
              "http://localhost:8081/Localidad/" + this.editedItem.id,
              {
                codigo: this.editedItem.id,
          ...
          axios
            .post("http://localhost:8081/Localidad", {
                codigo: this.editedItem.id,
                estado: this.editedItem.estado,
                descripcion: this.editedItem.descripcion,
                status: this.editedItem.status,
      ...
      deleteMapping: function (id) {
        //console.log(id)
        axios
          .delete("http://localhost:8081/Localidad/" + id)
          .then(() => {
            this.showData();
          });
      },
